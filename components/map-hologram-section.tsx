...
if (typeof window !== "undefined") {
  console.log(
    "Google Maps API Key:",
    GOOGLE_MAPS_API_KEY ? "Configured" : "Not configured"
  );
}
...
export default function MapHologramSection() {
  ...
  const FancyGlobe = useMemo(
    () => dynamic(() => import("./fancy-globe"), { ssr: false }),
    []
  );
      ...
      try {
        ...
        for (const row of data || []) {
          ...
          // PostGIS Geography WKB format (hex string)
            ...
            typeof c === "string" &&
            c.startsWith("0101000020E6100000")
          ) {
            try {
              ...
              console.warn(
                `[Map] Failed to parse WKB coordinates for ${(row as any).id}:`,
                (wkbErr as Error).message
              );
            }
          }
          ...
          if (
            ...
            console.warn(
              `[Map] Skipping parcel ${(row as any).id} - invalid coordinates:`,
              c
            );
          }
        }
  ...
  useEffect(() => {
    const handlePortfolioLandSearch = async (event: CustomEvent) => {
      ...
      try {
        ...
        console.warn(
          "[v0] Portfolio geocoding failed:",
          (geoErr as Error).message
        );
    ...
    window.addEventListener(
      "portfolioLandSearch",
      handlePortfolioLandSearch as unknown as EventListener
    );
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      try {
        ...
        const primary =
          theme.getPropertyValue("--color-primary")?.trim() || "#00e5ff";
        const accent =
          theme.getPropertyValue("--color-chart-2")?.trim() || "#ff3fd8";
        ...
        console.error(
          "[v0] Google Maps globe init error:",
        ...
        if (errorMessage.includes("BillingNotEnabledMapError")) {
          setMapsError(
            "Google Maps billing is not enabled. Please enable billing in your Google Cloud Console to use the Maps API. Check GOOGLE_MAPS_BILLING_SETUP.md for detailed instructions."
          );
        } else if (errorMessage.includes("RefererNotAllowedMapError")) {
          setMapsError(
            "This domain is not authorized to use the Google Maps API. Please add localhost:3000 to your allowed referrers."
          );
          ...
          setMapsError(
            "Google Maps is not available. Please check your API key configuration and billing settings."
          );
        }
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (searchResult?.parcelId) {
        const target = geojson.find(
          (g) => g.properties.id === searchResult.parcelId
        );
        if (target) {
          // Find and highlight the polygon for this parcel
          const targetPolygon = polygons.find(
            (poly) => (poly as any).__parcelId === searchResult.parcelId
          );
          if (targetPolygon) {
          ...
          const centroid = target.path.reduce(
            (acc, p) => ({
              lat: acc.lat + p.lat / target.path.length,
              lng: acc.lng + p.lng / target.path.length,
            }),
            { lat: 0, lng: 0 }
          );
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      if (supabase) {
        let query = supabase
          .from("property_listings")
          .select("id, seller_name, location, coordinates")
          .limit(1);
        if (parcel) {
        ...
        if (error) {
          ...
          // PostGIS Geography WKB format (hex string)
            ...
            try {
              ...
              coords = [lng, lat];
              console.log(
                `[Map] Parsed WKB coordinates for ${parcelId}: lat=${lat}, lng=${lng}`
              );
            } catch (wkbErr) {
              console.warn(
                `[Map] Failed to parse WKB coordinates for ${parcelId}:`,
                (wkbErr as Error).message
              );
            }
          }
          ...
          if (
            ...
            console.log(
              `[Map] Extracted coordinates for ${parcelId}: lat=${lat}, lng=${lng}`
            );
          } else {
            console.warn(
              `[Map] Invalid coordinates format for parcel ${parcelId}:`,
              c
            );
          }
          ...
          if (!targetLatLng) {
            setSearchError(
              `Parcel "${parcelId}" found but coordinates are missing or invalid.`
            );
            return;
          }
        } else {
    ...
    if (!parcelId && parcel) {
      const match = geojson.find((g) =>
        g.properties.id.toLowerCase().includes(parcel.toLowerCase())
      )?.properties.id;
      if (match) parcelId = match;
    }
    if (!parcelId && owner) {
      const match = geojson.find((g) =>
        g.properties.owner.toLowerCase().includes(owner.toLowerCase())
      )?.properties.id;
      if (match) parcelId = match;
    }
    ...
    try {
      ...
      const endLatLng =
        ...
        (() => {
          const byId =
            (parcelId && geojson.find((g) => g.properties.id === parcelId)) ||
            null;
          if (!byId) return undefined;
          const c = byId.path.reduce(
            (acc, p) => ({
              lat: acc.lat + p.lat / byId.path.length,
              lng: acc.lng + p.lng / byId.path.length,
            }),
            { lat: 0, lng: 0 }
          );
          return c;
      ...
      {!hasQuery && (
        <div className="mx-auto max-w-4xl px-6 py-10 md:py-14">
          <div className="glass holo-border scanlines rounded-lg p-6">
            <h2 className="text-balance text-2xl font-semibold holo-glow">
              DhartiLink Parcel Search
            </h2>
            <p className="mt-2 text-sm text-muted-foreground">
              Provide any one: Parcel ID, Owner name, or Address/Area. We’ll
              locate the parcel and reveal its hologram.
            </p>
            <form
            ...
            >
              ...
              {searchError && (
                <div className="md:col-span-3 text-sm text-destructive-foreground/90">
                  {searchError}
                </div>
              )}
            </form>
      ...
      {hasQuery && (
        <div className="sticky top-0 z-10 mx-auto w-full max-w-7xl px-4 pt-4">
          ...
          >
            ...
            {searchError && (
              <div className="md:col-span-4 text-xs text-destructive-foreground/90">
                {searchError}
              </div>
            )}
          </form>
              ...
              <div className="text-center space-y-4">
                ...
                <div className="space-y-2 text-sm text-gray-500">
                  ...
                  <ol className="list-decimal list-inside space-y-1 text-left">
                    ...
                    <li>
                      Add a payment method (Google provides $200 free credits)
                    </li>
                    <li>
                      Return to{" "}
                      <a
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    💡 <strong>Note:</strong> Google Maps API requires billing
                    to be enabled, even for development use.
                  </p>
                </div>
                <Button
                  onClick={() => {
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="glass holo-border p-4">
            <p className="text-xs leading-relaxed text-muted-foreground">
              {hasQuery
                ? "Hover parcels to preview details. Click to pin."
                : "Provide details to reveal parcels over the map."}
            </p>
