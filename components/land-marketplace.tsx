  ...
  useEffect(() => {
    const loadListings = async () => {
      try {
        if (!supabase) {
          console.warn(
            "[LandMarketplace] Supabase client not initialized. Check env vars."
          );
          toast.error(
            "Supabase not configured. Check .env and restart dev server."
          );
          return;
        }
        const { data, error } = await supabase
          ...
          .select(
            "id,title,description,price,area,location,seller,seller_name,listed_date,coordinates"
          );

        if (error) {
          console.error(
            "[LandMarketplace] Supabase query error:",
            error?.message || error,
            {
        ...
        const mapped: LandListing[] = (data || []).map((row: any) => {
          ...
          if (typeof sellerRaw === "string") {
            sellerStr = sellerRaw.startsWith("\\x")
              ? `0x${sellerRaw.slice(2)}`
              : sellerRaw;
          } else {
          ...
          if (
            c &&
            typeof c === "object" &&
            Array.isArray((c as any).coordinates)
          ) {
        ...
        if ((data || []).length === 0) {
          console.info(
            "[LandMarketplace] Supabase returned 0 rows from property_listings"
          );
    ...
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {listings.map((listing) => (
          <Card
            key={listing.id}
            className="glass holo-border group hover:scale-105 transition-all duration-300"
          >
            <CardHeader>
